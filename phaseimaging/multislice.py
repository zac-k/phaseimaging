...
npmax = 12
nzmin = 1
nzmax = 103  # Maximum allowed atomic number
ncmax = 132
nl = 3
ng = 3
SMALL = 1e-25
...
def project_phase_ms(axis, angle, wavelength, width, res, location_array):
    ...
    print("Calculate lii...")
    integrated_intensity = calculate_integrated_intensity(wave)

    print("Integrated Intensity: ", integrated_intensity)

    return fft.ifftshift(wave)

def build_atom_locations(specimen, width):
    """
...
def trlayer(x, y, z_number, na, aA, trans, window, fparams, wavelength):
    ...
    tratoms(trans, x, y, z_number, scalex, scaley, idx, rmax2, rminsq, na, fparams)
...
def tratoms(trans, x, y, z_number, scalex, scaley, idx, rmax2, rminsq, na, fparams):
    res = np.shape(trans)
    M = res[0]  # todo: make this work with rectangular arrays
    ixo = x / scalex
    iyo = y / scaley
    ixo = ixo.astype(int)
    iyo = iyo.astype(int)
